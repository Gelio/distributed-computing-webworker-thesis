...
\chapter{Analiza problemu}
    ...
    \section{Cel projektu}
        Celem projektu jest zaprojektowanie oraz zaimplementowanie systemu, który pozwoli na realizowanie obliczeń rozproszonych na wielu urządzeniach, na których dostępna jest jedynie przeglądarka internetowa. Tak utworzony system obliczeń rozproszonych powinien zagwarantować bardzo prostą konfigurację oraz szeroką wieloplatformowość. Aby rozwiązanie było konkurencyjne pod względem wydajności zdecydowano, że obliczenia w przeglądarce powinny być wykonywane w technologii \textit{WebAssembly} \cite{webassembly}. 
        
        Projekt przewiduje również przygotowanie biblioteki utworzonej w języku C\# ułatwiającej tworzenie nowych projektów uruchamianych w ramach systemu obliczeń rozproszonych.
        Takie rozwiązanie ma na celu zmniejszenie nakładu pracy wymaganej przy dodawaniu nowego algorytmu uruchamianego na węzłach obliczeniowych.
        
        Przygotowany system powinien także reagować na odłączanie się węzłów obliczeniowych w trakcie wykonywania zadania oraz umożliwiać wielokrotnie przeprowadzenie tych samych obliczeń w celu wybrania najbardziej wiarygodnego wyniku w razie rozbieżności.
    ...
    \section{Opis istniejących rozwiązań}
        W sieci Internet dostępne są obecnie rozwiązania umożliwiające wykonywanie obliczeń rozproszonych w przeglądarce internetowej, ale brakuje rozwiązań umożliwiających implementację algorytmów w języku C\#, a następnie wykonywanie obliczeń w przeglądarce internetowej z wykorzystaniem \textit{WebAssembly}.
        
        W miarę redukcji wymagań można odnaleźć rozwiązania, które częściowo pozwalają na realizację projektu. W momencie, gdy projekt nie wymaga umożliwienia implementacji algorytmów w przestępnym języku, a jedynie dostarczenia plików działających już w \textit{WebAssembly}, to można odnaleźć w sieci systemy, które również starają się rozwiązać ten problem. W chwili powstawania pracy inżynierskiej nie został odnaleziony jednak żaden system, który wyszedłby poza fazę testową i w pełni wspierał wykonywanie obliczeń rozproszonych zgodnie z założeniami projektu.
        
        W chwili powstawania powyższego dokumentu jedynie biblioteka \textit{mono-wasm} umożliwiała uruchamianie kodu napisanego w języku C\# w \textit{WebAssembly}. Ze względu na eksperymentalny stan tej biblioteki kompilacja C\# do \textit{WebAssembly} nie była możliwa, lecz \textit{mono-wasm} udostępniało interpreter C\# działający w \textit{WebAssembly}.
        
		\section{Wymagania funkcjonalne}
                
            Na rysunku nr \ref{use-cases-diagram} przedstawiono w postaci diagramu UML zbiór przypadków użycia systemu dla aktora będącego użytkownikiem zaprojektowanego systemu oraz dla aktora będącego węzłem obliczeniowym.
            ...
            W tabeli nr \ref{wymagania-funkcjonalne} zebrano opisy poszczególnych przypadków użycia.
            
            \pagebreak
            \begin{center}
                \begin{longtable}{| p{.15\textwidth} | p{.20\textwidth} | p{.30\textwidth} | p{.30\textwidth} |}
                    ...
                    & Lista zawierająca zakończone zadania. \\ \cline{1-4} 
                    
                    Użytkownik & Usuń zadanie
                    & Usunięcie zadania.
            ...
            W tabeli nr \ref{wymagania-niefunkcjonalne} przedstawiono wymagania niefunkcjonalne pogrupowane w poszczególne kategorie URPS.
            
            \begin{center}
                \begin{longtable}{| p{.20\textwidth} | p{.20\textwidth} | p{.60\textwidth} |}
                    ...
                    & \stepcounter{WymaganiaNiefunkcjonalne} \arabic{WymaganiaNiefunkcjonalne}
                    & Aplikacja powinna wykrywać rozbieżne wyniki podzadania zwrócone przez kilka węzłów obliczeniowych i kierować podzadanie do ponownego wykonania. \\ \hline
                    ...
                    & \stepcounter{WymaganiaNiefunkcjonalne} \arabic{WymaganiaNiefunkcjonalne}
                    & Aplikacja powinna zachować wsteczną zgodność z zadaniami zdefiniowanymi przez użytkowników. \\ \hline
