...
W ramach tej pracy inżynierskiej stworzono system umożliwiający realizację obliczeń rozproszonych
z węzłami działającymi w przeglądarce internetowej.
Obliczenia rozproszone są metodą na rozwiązanie potencjalnie skomplikowanych problemów dzieląc je na mniejsze
i rozwiązując każdy z nich równolegle na oddzielnych maszynach. Tradycyjnie dołączenie nowych urządzeń jako
węzły sieci obliczeń rozproszonych odbywało się poprzez instalację specjalnego oprogramowania klienckiego,
które komunikowało się z centralnym serwerem rozdzielającym zadania.

Uruchamianie węzłów obliczeniowych w przeglądarce internetowej uprościło proces dołączania nowych urządzeń
do sieci --- nie trzeba instalować oprogramowania węzła działającego bezpośrednio w systemie operacyjnym.
Dodatkowo, dzięki takiemu rozwiązaniu można wykorzystać więcej urządzeń jako węzły obliczeniowe z uwagi na
fakt, że urządzenia takie jak telewizory \textit{SmartTV} czy telefony komórkowe również mogą wykonywać zadania,
o ile ich przeglądarka internetowa spełnia wymagania systemowe.

Utworzone rozwiązanie zapewnia łatwe wykorzystanie modułów napisanych i kompilowanych
w języku C\#. Węzły obliczeniowe uruchamiają program klienta działający w technologii \textit{\selectlanguage{english}WebAssembly}
i komunikują się z centralnym serwerem rozdzielającym zadania i scalającym wyniki kiedy już zostaną odesłane.
Dodatkowo udostępniony został
panel administracyjny pozwalający na zarządzanie systemem.
Osiągnięta została założona wieloplatformowość węzłów obliczeniowych.
System pozwala na podłączenie komputerów z różnymi systemami operacyjnymi,
telefonów komórkowych oraz elektronicznych czytników książek jako 
węzły obliczeniowe.
Wydajność stworzonego rozwiązania jest gorsza od natywnej z powodu wykorzystania interpretera
języka C\# działającego w \textit{\selectlanguage{english}WebAssembly}.
To ograniczenie prawdopodobnie zniknie w przyszłych wersjach
\textit{\selectlanguage{english}mono-wasm}. \\



\noindent \textbf{Słowa kluczowe:} obliczenia rozproszone, WebAssembly, aplikacja internetowa, wydajność, obliczenia równoległe, przeglądarka internetowa, Internet
\end{abstract}
}
...
In this thesis we present a system for distributed computing
with nodes running in a web browser.
Distributed computing is a way to solve potentially complex problems by dividing them into
smaller parts and solving each of them in parallel using multiple machines.
Typically, adding new machines as distributed nodes was done by installing a native client
application responsible for communicating with a server that assigned the tasks.

Using a web browser as a client application simplified the process of adding new nodes
to~the network --- it is not necessary to install any operating system-level applications.
Additionally, thanks the proposed solution more devices can join the network as computing nodes.
That is~due to the fact that devices such as \textit{SmartTVs} or mobile phones can also
compute tasks, given that their web browser meets the requirements.

The created solution allows easy use of modules written and compiled in C\#.
Distributed nodes run the web client using \textit{WebAssembly} and communicate
with the server that assigns tasks to them and joins the results once they arrive.
Moreover, an administrator's panel has been developed to manage the system.
The planned cross-platform functionality has been achieved.
The system supports connecting PCs with various operating systems, smartphones,
as~well~as~e-book readers as computing nodes.
A C\# interpreter was used to run the code in \textit{WebAssembly}.
This resulted in achieving worse performance than running the same code natively.
This limitation is~likely to~be solved in the future versions of \textit{mono-wasm}. \\
...
\chapter{Wstęp}
    ...
    Przykładem klastra obliczeniowego jest DEGIMA~\cite{DEGIMA} (\textit{\selectlanguage{english}DEstination for Gpu Intensive Machine}).
    ...
    Jednym z takich projektów jest GIMPS~\cite{GIMPS} (\textit{\selectlanguage{english}Great Internet Mersenne Prime Search}).
    ...
    Projektem o podobnym założeniu jest SETI@home~\cite{SETI} (\textit{\selectlanguage{english}\textit{Search for Extraterrestial Intelligence}}).
    ...
    Węzły obliczeniowe próbują dodać nowy blok do tzw. \textit{\selectlanguage{english}blockchain}.
    ...
    jest w języku {\selectlanguage{english}Javascript}, który ze względu na swoją dynamiczność nie jest najlepszym wyborem
    jeżeli chodzi o obliczenia, w których liczy się wydajność. Na pomoc w tej sytuacji przychodzi
    eksperymentalna technologia \textit{\selectlanguage{english}WebAssembly}~\cite{webassembly-goals}.
    ...
    Poza stosem dostępna jest także pamięć liniowa (ang. \textit{\selectlanguage{english}linear memory}).
...
\chapter{Analiza problemu}
    ...
    \section{Opis istniejących rozwiązań}
		...
		\section{Wymagania funkcjonalne i niefunkcjonalne}
            ...
            \begin{longtable}{| p{.20\textwidth-2\tabcolsep} | p{.20\textwidth-2\tabcolsep} | p{.30\textwidth-2\tabcolsep} | p{.30\textwidth-2\tabcolsep} |}
            ...
            \begin{longtable}{| p{.20\textwidth-2\tabcolsep} | p{.20\textwidth-2\tabcolsep} | p{.60\textwidth-2\tabcolsep} |}
...
\begin{figure} \centering{
        \includegraphics[scale=0.75]{images/server-class-diagram-1.pdf}}
    \caption{Diagram klas serwera.}
    \label{server-class-1}
\end{figure}

\begin{figure} \centering{
        \includegraphics[scale=0.75]{images/server-class-diagram-2.pdf}}
...
\begin{figure} \centering{
        \includegraphics[scale=0.60]{images/subtask-state-diagram.pdf}}
...
\begin{figure} \centering{
        \includegraphics[scale=0.60]{images/sequence-diagram.pdf}}
...
\begin{figure} \centering{
        \includegraphics[scale=0.6]{images/node-state-diagram.pdf}}
...
\section{Metoda wytwórcza}
    ...
    Podczas tworzenia projektu zastosowany był kaskadowy model wytwarzania oprogramowania (ang. \textit{\selectlanguage{english}waterfall model}).
    ...
    Na końcu przygotowano testy integracyjne całego systemu (ang. \textit{\selectlanguage{english}end-to-end tests}),
    ...
    do głównej gałęzi repozytorium (ang. \textit{\selectlanguage{english}pull request}).
...
\section{Podział prac}
    ...
    \begin{longtable}{| p{.30\textwidth-2\tabcolsep} | p{.70\textwidth-2\tabcolsep} |}
...
\chapter{Opis aplikacji}
    ...
    \begin{longtable}{| p{.10\textwidth-2\tabcolsep} | p{.20\textwidth-2\tabcolsep} | p{.50\textwidth-2\tabcolsep} | p{.20\textwidth-2\tabcolsep} |}
   ...
   # docker-compose -f docker-compose.yml -f docker-compose.prod.yml stop
    ...
    hasło: D1stributed$
        ...
        Węzły sieciowe mogą w każdym momencie przerwać obliczenia przez kliknięcie przycisk \texttt{Stop the worker} oraz dowolnie konfigurować ilość udostępnianych wątków. 
    ...
    \section{Testy akceptacyjne}
        ...
        \subsection{Użytkownik anonimowy}
            ...
            \paragraph{Zwiększenie ilości wątków.}   
                ...
                Aby zwiększyć ilość wątków dostępnych do wykonywania obliczeń należy po uprzednim uruchomieniu węzła obliczeniowego kliknąć przycisk z ikoną plusa.
            ...
            \paragraph{Zmniejszenie ilości wątków.}   
                ...
                Aby zmniejszyć ilość wątków dostępnych do wykonywania obliczeń należy po uprzednim uruchomieniu węzła obliczeniowego oraz zwiększeniu liczby używanych wątków kliknąć przycisk z~ikoną minusa.
...
\chapter{Podsumowanie}
    ...
    \section{Wnioski z projektu}
        ...
        \subsection{Dokładna weryfikacja wymagań}
            ...
            Na początku projektu ustalono, że taka funkcjonalność jest dostępna w {\selectlanguage{english}\textit{.NET Standard 2.0}} z użyciem klasy {\selectlanguage{english}\texttt{AppDomain}}. Wykorzystując {\selectlanguage{english}\textit{.NET Core 2.2}}, który implementuje {\selectlanguage{english}\textit{.NET Standard 2.0}} oczekiwane było, że wymagana funkcjonalność będzie dostępna w naszym projekcie.
            Ku naszemu zdziwieniu okazało się jednak, że mimo, że takie rozwiązanie ({\selectlanguage{english}\texttt{AppDomain}}) jest w teorii w {\selectlanguage{english}\textit{.NET Standard 2.0}}, to nie jest wspierane w {\selectlanguage{english}\textit{.NET Core}}, zatem konieczna była rezygnacja z tego wymagania.
...
\begin{tabular}{p{.15\textwidth-2\tabcolsep} p{.85\textwidth-2\tabcolsep}}
    API 
