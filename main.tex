...
\chapter{Wstęp}
    ...
    Obliczenia równoległe to model obliczeń pozwalający na pracę wielu wątków nad tym samym problemem w celu zdobycia wyników w krótszym czasie, niż jakby dane zadanie było wykonywane w sposób sekwencyjny przez jeden wątek.
    ...
    Do obliczeń równoległych należy stosować algorytmy równoległe, które często okazują się
    ...
    Wykorzystanie wielu wątków do obliczeń jest szczególnie ważne w dzisiejszych czasach, gdy częstotliwość taktowania procesorów nie wzrasta w tempie wykładniczym,
    jak było to w poprzednich latach i co jest opisane przez prawo Moore'a~\cite{prawo-moorea}.
    Wydajność algorytmów można zwiększyć w znacznym stopniu jedynie poprzez zlecenie ich wykonywania równolegle,
    ...
    Często do komunikacji pomiędzy węzłami uczestniczącymi w obliczeniach rozproszonych używa się sieci Internet.
    ...
    Jednym z konkretnych modeli wykorzystujących obliczenia rozproszone jest przygotowanie klastra obliczeniowego.
    Jest to grupa połączonych ze sobą komputerów, często o podobnych parametrach, umiejscowionych zazwyczaj w tym samym budynku.
    Często takie rozwiązania są stosowane do celów naukowych.
    Taki klaster jest dostępny dla naukowców do uruchamiania na nim algorytmów rozproszonych.
    ...
    Obliczenia rozproszone działają również w modelu klient-serwer.
    ...
    To serwer jest odpowiedzialny za ewentualną agregację wyników wielu rozdzielonych zadań.
    ...
    % Może tą sekcję wyrzucić, skoro opisałem już GIMPS
    ...
    Węzły podłączone do tego systemu otrzymują próbki sygnału z radioteleskopu i starają się znaleźć w nich sygnału od pozaziemskich cywilizacji.
    ...
    % Tu zaczyna się motywacja. Czy to powinno być osobną sekcją?
    ...
    Problemem może okazać się wydajność. Kod uruchamiany najczęściej w przeglądarce napisany
    jest w języku {\selectlanguage{english}Javascript}, który jest językiem interpretowanym i nie jest najlepszym wyborem
    ...
    Za wykonanie kodu WebAssembly odpowiedzialna jest wirtualna maszyna stosowa.
    Instrukcje wykonywane sekwencyjnie mogą modyfikować szczyt stosu, usuwać lub dodawać elementy.
    Istnieją też instrukcje kontrolne pozwalające na skok w inne miejsce w kodzie.
    ...
    W ramach pracy inżynierskiej stworzony został system do obliczeń rozproszonych pozwalający
    ...
    Jako język do implementacji algorytmów rozwiązujących problemu został wybrany język C\#.
    Posiada on dużą ilość pakietów pomocnych przy implementacji operacji matematycznych,
    ...
    Zbudowane z kodu biblioteki DLL są przetwarzane do postaci możliwej do uruchomienia w
    WebAssembly za pomocą narzędzia \textit{mono-wasm}.
    
    Poza bazową funkcjonalnością jaką jest kontrolowanie obliczeń, stworzony system zawiera
    środki bezpieczeństwa, które utrudnią bądź zapobiegną szkodliwym akcjom, które mogą chcieć
