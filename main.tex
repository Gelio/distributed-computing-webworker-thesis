...
\chapter{Analiza problemu}
    ...
    \section{Cel projektu}
        ...
        Projekt przewiduje również przygotowanie bibliotek utworzonych w języku \textit{C\#} dostarczanych przez programistów do uruchomienia w \textit{WebAssembly}. Takie rozwiązanie ma na celu zmniejszenie nakładu pracy wymaganej przy dodawaniu nowego algorytmu uruchamianego na węzłach obliczeniowych.
        
        Przygotowany system powinien także reagować na odłączanie się węzłów obliczeniowych w trakcie wykonywania zadania oraz umożliwiać wielokrotnie przeprowadzenie tych samych obliczeń w celu wybrania najbardziej wiarygodnego wyniku.
    ...
    \section{Opis istniejących rozwiązań}
        W sieci Internet dostępne są obecnie rozwiązania umożliwiające wykonywanie obliczeń rozproszonych w przeglądarce internetowej, ale brakuje rozwiązań umożliwiających implementację algorytmów w języku \textit{C\#}, a następnie wykonywanie obliczeń w przeglądarce internetowej z wykorzystaniem \textit{WebAssembly}.
        ...
        W chwili powstawania powyższego dokumentu jedynie biblioteka \textit{mono-wasm} umożliwiała uruchamianie kodu napisanego w języku \textit{C\#} w \textit{WebAssembly}. Ze względu na eksperymentalny stan tej biblioteki kompilacja \textit{C\#} do \textit{WebAssembly} nie była możliwa, lecz \textit{mono-wasm} udostępniało interpreter \textit{C\#} działający w \textit{WebAssembly}.
